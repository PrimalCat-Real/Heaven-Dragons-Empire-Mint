...
import NftImage from '../src/img/HvnDragon_2.gif';
...
const Home = (props: HomeProps) => {
    ...
    const renderGoLiveDateCounter = ({days, hours, minutes, seconds}: any) => {
        if(hours >= 0){
            return (
                <Card>{hours +((24*days))}:{minutes}:{seconds}</Card>
            );
        }else{
            return (
                <Card>00:00:00</Card>
    ...
    const renderGoLiveDateCounter2 = ({days, hours, minutes, seconds}: any) => {
            if(hours == 0 && days >= 0){
                console.log(hours == 0 && days >= 0, "my hours")
                return(
                    <Card>{hours}:{minutes}:{seconds}</Card>
                )
                
            }else if(hours == 0 && days == 0 && minutes == 0 && seconds == 0){
    ...
    function JustLogen(){
        var stable = toDate(candyMachine?.state.goLiveDate)
        let hours = new Date().getHours();
        let min = new Date().getMinutes();
        let sec = new Date().getSeconds();
        let data = new Date().getDate();
        if(stable != null && stable != undefined) {
            console.log((stable.getHours() <= hours || stable.getMinutes()+ 5 <= min) && data >= stable.getDate()-2, "my");
        }
    ...
    function setInt(){
        ...
        let data = new Date().getDate();
        if(stable != undefined) {
            var firth_step = stable.getHours() <= hours
            var last_step =  stable.getMinutes() + 3 <= min
            var step_4 = stable.getMinutes()+ 2 <= min && stable.getSeconds()+ 20 <= sec
            var step_3 = stable.getMinutes()+ 2 <= min
            var step_2 = stable.getMinutes()+ 1 <= min && stable.getSeconds()+ 30 <= sec
            var step_1 = stable.getMinutes()+ 1 <= min
            var perstep = stable.getSeconds()+ 25 <= sec && stable.getMinutes() <= min
            if((stable.getHours()+1 <= hours || stable.getMinutes()+ 5 <= min) && data >= stable.getDate()-2){
                value = 27.3;
                setLevel(27.3)
                return value
            }else if(step_4 && last_step && data >= stable.getDate()-2){
                value = 27.3;
                setLevel(27.3)
                return value
            }else if(step_3 && data && firth_step && data >= stable.getDate()-2){
                value = 20.7;
                setLevel(value)
                return value
            }else if(step_2 && data && firth_step && data >= stable.getDate()-2){
                value = 14.1;
                return value
            }else if((step_1 && firth_step && data >= stable.getDate()-2)){
                value = 8.4;
                return value
            }else if((firth_step && perstep && data >= stable.getDate()-2)){
                value = 4.3;
                return value
            }else{
                return value
    ...
    // function Data(){
    //     var my_date = toDate(candyMachine?.state.goLiveDate)
    //     return (my_date)
    // }
    useEffect(() => {
        const timer = setInterval(()=>{
            setLevel((newLevel) => setInt());
        }, 5000);
        return () => {
            // clearInterval(timer);
            // Welcome()
            setLevel(setInt())
            // console.log(setInt(), "my value")
        }
    })
    // function Welcome() {
    //     var stable = toDate(candyMachine?.state.goLiveDate)
    ...
    //     let sec = new Date().getSeconds();
    //     if(stable != null || stable != undefined) {
    //         if((stable?.getHours() -1) == hours && stable?.getMinutes() <= min + 2){
    //             return 5
    //         }else if((stable?.getHours() -1) == hours && stable?.getMinutes() <= min + 1){
    //             return 15
    //         }else if((stable?.getHours() -1) == hours && stable?.getMinutes() <= min){
    //             return 10
    //         }else if((stable?.getHours() -1)  == hours){
    //             return 5
    //         }else{
    //             return 0
    //         }
...
The Heaven Dragons Empire collection aims to create an staking game with different types of attributes to its staking. H.D.E objective is to create an elite and active community where everyone can help each other.</DragonInfoDesc>
                            ...
                            >
                            <InfoCount>{isActive && whitelistEnabled && (whitelistTokenBalance > 0) ? (whitelistPrice): "2"} SOL</InfoCount>
                            <InfoCountSub>WL Price </InfoCountSub>
                            </DragonInfoArtical>
                        </DragonInfoSection>
                        </DragonInfo>
                ...
                <WalletContainer>
                    <NftImageBlock>
                        ...
                        <div style={{ 
                            ...
                            variant="determinate"
                            value={(100 - ((itemsRemaining - level*10) * 100 / (itemsAvailable)))}
                            ></BorderLinearProgress>
                            <Typography
                                style={{
                                    position: "absolute",
                                ...
                                >
                                {(itemsAvailable+level*10) - itemsRemaining}/{1525}
                                </Typography>
                        </div> 
